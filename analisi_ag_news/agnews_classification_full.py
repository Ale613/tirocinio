...
def classificazione(X_train, X_test, y_train, y_test, file):
    ...
    calculateResults(nameClassifier='Xgboost', y_test=y_test_xb, y_pred=y_pred_xb, file=file)

from datasets import load_dataset

ag_news = load_dataset("ag_news")

df_agNewsTrain = pd.DataFrame()
df_agNewsTrain['data'] = ag_news['train']['text']
df_agNewsTrain['target'] = ag_news['train']['label']

df_agNewsTest = pd.DataFrame()
df_agNewsTest['data'] = ag_news['test']['text']
df_agNewsTest['target'] = ag_news['test']['label']

# train
X_train = df_agNewsTrain['data']
y_train = df_agNewsTrain['target']
# test
X_test = df_agNewsTest['data']
y_test = df_agNewsTest['target']

#TD-IDF Full Test
f = open("results_TFIDF_agnews_full.txt", "w")
...
# RoBERTa Full Test
f = open("results_RoBERTa_agnews_full.txt", "w")
...
f = open("results_mpnet_agnews_full.txt", "w")
...
# ALBERT Full Test
f = open("results_ALBERT_agnews_full.txt", "w")
